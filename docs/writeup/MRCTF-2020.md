...
![GifRPO.png](https://s1.ax1x.com/2020/03/27/GifRPO.png)
+ 第二种解法

用`stegsolve`查看图片，`Save Bin`可直接得到图片：

![GZhFyT.png](https://s1.ax1x.com/2020/03/29/GZhFyT.png)
...
![GifOJS.png](https://s1.ax1x.com/2020/03/27/GifOJS.png)
...
![GihFiT.png](https://s1.ax1x.com/2020/03/27/GihFiT.png)
...
萌新有$n$本经验书，分成了若干堆（含一堆），每堆经验书不少于$l$本且不超过$r$本。

萌新将所有数量相同的堆分为一类，并将堆数不少于$k$的类分别装箱存放。

所有经验书都是一样的。

萌新想知道，所有划分方案需要的箱子数的总和（对 65536 取模）。

每组询问包括四个整数 $n,k,l,r$。只要计算出萌新的所有询问，即可得到 flag。


#### Solution

比赛前一天强行魔改了一道以前出的NOIP模拟赛题目。难度NOIP+省选-吧。

因为没有时限，数据范围改成了$10^8$，$O(n^{1.5})$大概跑半小时以内吧（最后几组数据跑一起算）。

记$n$有$p_n$种不同的划分，数字$i$出现至少$k$次的划分有$p_{n-ik}$个，一个划分正好会被每个至少出现了$k$次的数字计算一次，所以答案就是$$\sum_{i=l}^r p_{n-ik}$$

时间复杂度$\mathcal{O}(n(r-l))$。

当$l \geq \sqrt{n}$时，最多只会选$\sqrt{n}$个数。

$f_{i,j}$表示$j$划分为$i$个$l$至$r$间的数之和的方案数，有$$f_{i,j}=f_{i,j-i}+f_{i-1,j-l}-f_{i-1,j-i-r}$$

$f_{i,j-i}$表示将$i$件物品总体积为$j-i$的所有方案中的所有物品体积加一；

$f_{i-1,j-l}$表示将$i-1$件物品总体积为$j-l$的所有方案中加入一个体积为$l$的物品；

最后再减去出现了体积为$r+1$的物品的方案数，即$f_{i-1,j-i-r}$。

时间复杂度$\mathcal{O}(\frac{n^2}{l})$。

设置一个阈值$x$，$f_{i,j}$表示$j$划分为$i$个$x$至$r$间的数之和的方案数。

用完全背包求出$\{f_{0,n}\}$，即$n$划分为数个$l$至$x-1$中数之和的方案数；再套用解法三即可。

时间复杂度$\mathcal{O}(n(x-l)+\frac{n^2}{x})$。
$x$取$\mathcal{O}(\sqrt{n})$时，时间复杂度最优，为$\mathcal{O}(n^{1.5})$。
...
以下两种解法改了模数后我没想过，不知道能不能写。

#### 解法二

考虑生成函数。记$P(x)$为$\{p_n\}$的普通型生成函数，有
$$P(x)=\prod_{i=l}^r \frac{1}{1-x^i}$$

记$$G(x)=\sum_{i=0}^{\infty} g_i x^i=\prod_{i=l}^r (1-x^i)$$

则有$P(x)=G^{-1}(x)$，若求出$G(x)$，多项式求逆元即可求出$P(x)$。

可以看出，$\{g_n\}$为物品体积为$l$至$r$的01背包的偶数与奇数件物品的方案数之差。

显然至多选$\mathcal{O}(\sqrt{n})$件物品。令$h_{i,j}$表示$i$件物品总体积为$j$的方案数，有$$h_{i,j}=h_{i,j-i}+h_{i-1,j-(i-1)-l}-h_{i-1,j-r-1}$$

时间复杂度$\mathcal{O}(n^{1.5} + n \log n)$。

#### 解法三

[第三类欧拉变换](http://mathworld.wolfram.com/EulerTransform.html)。

$$P(x) = \prod_{i=1}^{\infty} \frac{1}{(1-x^i)^{a_i}}$$

记$A(x)$为$\{a_n\}$的普通型生成函数，有

$$P(x) = \exp \left(\sum_{i=1}^{\infty} \frac{A(x^i)}{i} \right)$$
...
>可以看下之前大佬发的文章
>推荐阅读
>https://processor.pub/2018/12/23/WASM%E7%9A%84%E5%AE%89%E5%85%A8%E6%80%A7%E9%97%AE%E9%A2%98/
...
### hello_world_go
Golang逆向
Golang程序去掉符号可以还原，ida下有脚本[IDAGolangHelper](https://github.com/sibears/IDAGolangHelper)帮助恢复符号，运行脚本后从上到下点一遍按钮就还原了2333
接下来分析程序逻辑
发现字符串比较，点进去看看
![](https://md.byr.moe/uploads/upload_3e729b3e9de3fbf8227bb1ce49c1de66.png)
![](https://md.byr.moe/uploads/upload_755b0aa31491696d6fa0a4f20ac66806.png)
...
NewCipher传参
![](https://md.byr.moe/uploads/upload_e047f54c3fd00b1e0045a59531f93924.png)
![](https://md.byr.moe/uploads/upload_56f1aa4427310a722267dc9dbd73be20.png)

move函数传参
![](https://md.byr.moe/uploads/upload_6b71a9505bd7c39a029ebb20d4e062a5.png)
![](https://md.byr.moe/uploads/upload_92b62fbb79112407c39844592b52d795.png)
![](https://md.byr.moe/uploads/upload_be8fbf384fb5b24aa413d9194838e260.png)
![](https://md.byr.moe/uploads/upload_c4596d1cfa038056a3fbd91636b2d881.png)
由此可以推测出它的传参规律
那个做出来的师傅是直接找的字符串比较位置把flag解出来的，这个...嗯..算是非预期，也是我大意了，我是想让大家去逆move这个函数逻辑的。
思路1(官方解法):
简单看一下move函数逻辑，往下一翻发现是递归，再看递归的函数调用
![](https://md.byr.moe/uploads/upload_cf33befc4e46b04200e20366c1b8e97c.png)

根据Golang自己的数据结构(这个我就没多研究了)可以推断接下来要执行的函数
move(arg0-1,arg1,arg3,arg2)
move(arg0-1,arg2,arg1,arg3)
又通过main中调用情况推断是汉诺塔，到网上抄一个汉诺塔下来编译一下验证一下就过了
思路2(非预期):
![](https://md.byr.moe/uploads/upload_d0ab349447de7d5cabdad95f7dd6f3a3.png)
通过这个compare反着下手
通过分析逻辑发现，flag被rc4加密后与加密后的输入进行比较，那我们获得加密后的数据再解密不就拿到flag了
借个师傅的[wp代码](https://www.ctfwp.com/%E5%AE%98%E6%96%B9%E8%B5%9B%E4%BA%8B%E9%A2%98/2020MRCTF)([lingze师傅的wp](https://lingze.xyz/20/03/mrctf-1ing23/)也很ok)
```
package main
